...
appSetup () {
	...
	NETBIOSDOMAIN=${NETBIOSDOMAIN:-SAMDOM}
	DOMAINADMIN=${DOMAINADMIN:-administrator}
	...
	LDOMAIN=${DOMAIN,,}
	UDOMAIN=${DOMAIN^^}
	URDOMAIN=$NETBIOSDOMAIN

    echo "dns domain: $LDOMAIN"
    echo "user domain: $URDOMAIN"
    echo "realm: $UDOMAIN"
	...
	if [[ ! -f /etc/samba/external/smb.conf ]]; then
		...
		if [[ ${JOIN,,} == "true" ]]; then
			samba-tool domain join ${LDOMAIN} DC -U"${URDOMAIN}\\${DOMAINADMIN}" --password="${DOMAINPASS}" --dns-backend=SAMBA_INTERNAL
		else
