...
envsubst '${WEBFQDN} ${TARGETCORONA} ${TARGETME} ${TARGETDOWNLOADS} ${TARGETMONITORING} ${TARGETKIBANA} ${TARGETAPTREPO}' < /etc/nginx/conf.d/default.conf.template > /etc/nginx/conf.d/default.conf
