...
envsubst '${WEBFQDN} ${TARGETCORONA} ${TARGETME} ${TARGETDOWNLOADS} ${TARGETMONITORING} ${TARGETAPTREPO}' < /etc/nginx/conf.d/default.conf.template > /etc/nginx/conf.d/default.conf
